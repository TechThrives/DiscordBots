...
function getDurationString(track) {
  ...
  if (!track.info.length) return "N/A";
...
module.exports = {
  ...
  nowPlaying: (target, track) => {
    ...
    const requesterTag =
      track.info.requester.globalName || `${track.info.requester.username}#${track.info.requester.discriminator || "0000"}`;
  ...
  help: (target, commands) => {
    const channelRestrictionNote = `\n\n**Channel Restrictions:**\n• Commands can only be used in <#${config.botSettings.commandsChannelId}>\n• Music can only be played in <#${config.botSettings.voiceChannelId}>`;
    const embed = new EmbedBuilder()
      .setColor(config.embedColor)
      .setTitle(`${emojis.info} Available Commands`)
      .setDescription(
        commands.map((cmd) => `${emojis.music} \`/${cmd.name}\` - ${cmd.description}`).join("\n") +
          channelRestrictionNote
      )
