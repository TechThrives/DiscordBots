...
const commands = [
  new SlashCommandBuilder()
    ...
    .addStringOption((option) => option.setName("query").setDescription("Song or playlist to play").setRequired(true)),
  ...
  new SlashCommandBuilder().setName("nowplaying").setDescription("Show current track info"),
  ...
  new SlashCommandBuilder().setName("loop").setDescription("Toggle queue loop mode"),

  new SlashCommandBuilder()
    .setName("remove")
    .setDescription("Remove a track from queue")
    .addIntegerOption((option) =>
      option.setName("position").setDescription("Position of the track to remove").setRequired(true)
    ),

  new SlashCommandBuilder().setName("clear").setDescription("Clear the current queue"),

  new SlashCommandBuilder().setName("status").setDescription("Show player status"),
...
client.on("interactionCreate", async (interaction) => {
  ...
  if (interaction.channelId !== config.botSettings.commandsChannelId) {
    return interaction.reply({
      content: `${emojis.error} | Commands can only be used in <#${config.botSettings.commandsChannelId}>!`,
      ephemeral: true,
    });
  }
  ...
  if (musicCommands.includes(commandName)) {
    ...
    if (interaction.member.voice.channelId !== config.botSettings.voiceChannelId) {
      return interaction.reply({
        content: `${emojis.error} | Music can only be played in <#${config.botSettings.voiceChannelId}>!`,
        ephemeral: true,
      });
    }
  }
